...
class Individual extends Equatable {
  ...
  Map<Team, List<Responsibility>> getAllAcceptedResponsibilities() =>
      _acceptedResponsibilities;
  List<Responsibility>? getAcceptedResponsibilities(Team team) =>
      _acceptedResponsibilities[team];
  Map<Team, List<Responsibility>> getAllNotAcceptedResponsibilities() =>
      _notAcceptedResponsibilities;
  List<Responsibility>? getNotAcceptedResponsibilities(Team team) =>
      _notAcceptedResponsibilities[team];
  List<Team> getTeams() => _teams;

  void joinTeam(Team team) {
    _teams.add(team);
    _acceptedResponsibilities.putIfAbsent(
        team, () => List.empty(growable: true));
  ...
  void acceptResponsibility(Team team, Responsibility res) {
    if (_teams.contains(team) &&
        _notAcceptedResponsibilities[team]!.contains(res)) {
